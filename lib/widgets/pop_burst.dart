...
class PopBurst extends StatefulWidget {
  ...
  static Duration get duration => GameConstants.burstLifetime;
...
class _PopBurstState extends State<PopBurst> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _particles = _BurstParticle.generate(
      count: GameConstants.burstParticleCount,
...
class _BurstParticle {
  ...
  static List<_BurstParticle> generate({
    ...
    return List.generate(count, (index) {
      final shapeRoll = random.nextInt(3);
      return _BurstParticle(
        angle: random.nextDouble() * pi * 2,
        color: colors[random.nextInt(colors.length)],
        size: 3 + random.nextDouble() * 14,
        shape: switch (shapeRoll) {
          0 => _ParticleShape.star,
          1 => _ParticleShape.circle,
          _ => _ParticleShape.strip,
        },
        spin: random.nextDouble() * pi * 2,
        speed: 0.6 + random.nextDouble() * 0.65,
...
class _BurstPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final opacity = (1 - Curves.easeIn.transform(rawProgress)).clamp(0.0, 1.0);

    if (rawProgress < 0.35) {
      final flashT = (1 - rawProgress / 0.35).clamp(0.0, 1.0);
      canvas.drawCircle(
        center,
        18 * flashT,
      ...
      canvas.drawCircle(
        center,
        10 * flashT,
        Paint()..color = accentColor.withOpacity(0.3 * flashT),
      );
    }

    if (rawProgress < 0.5) {
      final ringT = Curves.easeOut.transform(rawProgress / 0.5);
      final ringPaint = Paint()
        ..color = Colors.white.withOpacity((1 - ringT) * 0.28)
        ..style = PaintingStyle.stroke
        ..strokeWidth = 2;
      canvas.drawCircle(center, 10 + ringT * 32, ringPaint);
    }
    ...
    for (final particle in particles) {
      ...
      final particleScale = 0.5 + 0.5 * (1 - localProgress);
      final fall = gravity * pow(localProgress, 1.25);
      ...
      switch (particle.shape) {
        ...
        case _ParticleShape.strip:
          ...
          canvas.rotate(particle.angle + particle.spin + localProgress * pi * 2.5);
          canvas.drawRRect(
            RRect.fromRectAndRadius(
              Rect.fromCenter(
                center: Offset.zero,
                width: particle.size * particleScale,
                height: particle.size * 0.45 * particleScale,
              ),
  ...
  bool shouldRepaint(covariant _BurstPainter oldDelegate) {
    return oldDelegate.progress != progress || oldDelegate.rawProgress != rawProgress;
  }
}
